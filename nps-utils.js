// Copied / changed from unmaintained https://github.com/kentcdodds/nps-utils/blob/b20c1db9dd79945ee255eff27897d26f763dfd98/src/index.js
...
// opn-cli has vulnerabilities Severity: high trim-newlines  <3.0.1
// /**
...
function includePackage(packageNameOrOptions) {
  ...
  const relativeDir = path
    .relative(startingDir, upath.dirname(packageScriptsPath))
    .split('\\')
    .join('/')
...
function getBin(packageName, binName = packageName) {
  ...
  const concurrentlyDir = upath.dirname(packagePath)
  ...
  const fullBinPath = upath.join(concurrentlyDir, binRelativeToPackage)
  return upath.relative(process.cwd(), fullBinPath)
}
